...
namespace suhan_contact_planner
{
  ...
  void ContactOptimization::setOrigin(Eigen::MatrixXd origin)
  {
    origin_ = origin;
  }

  void ContactOptimization::setNormalVector(Eigen::Vector3d normal_vector)
  {
  ...
  void ContactOptimization::setTop(const double &contact_number, const double &contact_top_number, const std::vector<ContactPtr> &contacts, ContactOptimizationSolver &solver_top)
  {
    auto eq_constraint = std::make_shared<ConstraintEquality>();
    // eq_constraint.
    Eigen::MatrixXd A;
    ...
    Eigen::MatrixXd C_all;
    Eigen::VectorXd d_all;
    d_all.resize(12*(contact_top_number +1)+1);

    C_all.setZero(12 * (contact_top_number +1)+1, 6 * (contact_top_number +1));
    // d_all.setZero(contact_number * 12);
    ...
    Eigen::MatrixXd C_n(13, 6), d_n(13, 1);

    for (int i = 0; i < 6; i++)
    {
      C_n.block<2, 6>(i * 2, 0) = C_max[i];
      d_n.block<2, 1>(i * 2, 0) = d_max[i];
    }
    ...
    //C_n.block<1, 3>(12,0):remove R from (R^T*reslut_force)-> normal_vector_*result_force
    C_n.block<1, 3>(12,0) = normal_vector_.transpose();
    C_n.block<1,3>(12,3) << 0, 0, 0;

    C_all.block<13,6>(contact_top_number*12, contact_top_number*6) = C_n;
    d_all.segment<13>(contact_top_number * 12) = d_n;
  ...
  bool ContactOptimization::solve()
  {
    ...
    setTop(contact_number, contact_bottom_number, contacts, solver_top);
    ...
    if (solver_top.solve(result_top))
    {
      ...
      Eigen::VectorXd normal_force(6);
      normal_force = result_top.segment<6>(contact_top_number*6);
